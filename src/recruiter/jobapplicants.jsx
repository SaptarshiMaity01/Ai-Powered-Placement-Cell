...
const JobApplicants = ({ jobId }) => {
  ...
  const handleViewFile = (fileUrl) => {
    ...
    const formattedUrl = fileUrl.startsWith('http') ? fileUrl : `${process.env.REACT_APP_API_BASE_URL || ''}${fileUrl}`;
              ...
              {filteredApplications.length === 0 ? (
                ...
                filteredApplications.map((application) => (
                  <TableRow key={application._id}>
                    ...
                    <TableCell className="text-gray-600">
                      {application.linkedinProfile && application.linkedinProfile !== "Not provided" ? (
                        <a 
                          href={application.linkedinProfile.startsWith('https') ? 
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleViewFile(application.resume)}>
                              <Eye className="mr-2 h-4 w-4" />
                              View Resume
                            </DropdownMenuItem>
                            {application.coverLetterFile && (
                              <DropdownMenuItem onClick={() => handleViewFile(application.coverLetterFile)}>
                            ...
                            <DropdownMenuItem onClick={() => handleStatusChange(application._id, "review")}>
                              <Eye className="mr-2 h-4 w-4 text-purple-500" />
                              Mark for Review
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleScheduleInterview(application._id)}>
